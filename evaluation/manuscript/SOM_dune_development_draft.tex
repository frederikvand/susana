...
% Target journal: Ecological Engineering (Elsevier) -- see journal_options.md
% =============================================================================

%% Review mode: double-spaced, line numbers, preprint layout
\documentclass[authoryear,preprint,review,12pt]{elsarticle}

%% Packages already loaded by elsarticle: graphicx, amsmath, natbib
\usepackage{amssymb}
\usepackage{booktabs}
\usepackage{siunitx}
\usepackage{lineno}
...
\author[ugent-eng,biomorpho]{Frederik Van~Daele\corref{cor1}}
\ead{frederik.vandaele@ugent.be}
\author[ugent-bio]{Charlotte Taelman}
\author[ugent-bio]{Maaike Dhondt}
\author[kuleuven]{Jianghua Guo}
\author[ugent-bio]{Jiayue Yan}
\author[uantwerp]{Katrien Vanderbiest}
\author[kuleuven]{Pieter Rauwoens}
\author[ugent-bio]{Dries Bonte}

\cortext[cor1]{Corresponding author}

\affiliation[ugent-eng]{organization={Department of Civil Engineering},
    city={Ghent}, country={Belgium},
    note={Ghent University}}
\affiliation[biomorpho]{organization={Terrestrial Ecology Unit, Department of Biology},
    city={Ghent}, country={Belgium},
    note={Ghent University}}
\affiliation[ugent-bio]{organization={Terrestrial Ecology Unit, Department of Biology},
    city={Ghent}, country={Belgium},
    note={Ghent University}}
\affiliation[kuleuven]{organization={Department of Civil Engineering, Technology Campus Bruges},
    city={Bruges}, country={Belgium},
    note={KU Leuven}}
\affiliation[uantwerp]{organization={Department of Biology},
    city={Antwerp}, country={Belgium},
    note={University of Antwerp}}
...
%% TODO: Insert graphical abstract figure when available
%\includegraphics{figures/graphical_abstract}
\end{graphicalabstract}

\begin{highlights}
\item SOM content of alternative dredging sediments significantly affects marram grass establishment and vigour.
\item Moderate SOM enrichment (1--2\% TOC) optimises both vegetation growth and aeolian sand supply.
\item Living Dunes model extended with dynamic SOM--ecohydrology and SOM--cohesion pathways.
\item XBeach storm erosion modelling shows organic-enriched dunes have competitive storm resilience.
\item Evidence-based guidelines for sustainable reuse of dredging by-products in dune-in-front-of-dike systems.
\end{highlights}

\begin{keyword}
soil organic matter \sep nature-based solutions \sep dune restoration \sep marram grass \sep
alternative sediments \sep aeolian transport \sep coastal resilience \sep Living Dunes model \sep XBeach
\end{keyword}
...
The SOM response functions enter the geometric mean as additional environmental factors $f_{\SOM}(\cdot)$ for density growth and height growth (\cref{eq:gaussian_som}), parameterised from the treatment-mean fits described in \cref{sec:som_response_derivation}. The derived parameters for the Living Dunes vegetation model are listed in \cref{tab:som_derived_parameters}, and the modelled response curves are shown in \cref{fig:som_response_modelled}.
...
The number of living leaves showed a significant substrate effect in the Poisson GLMM. EMMs for leaf count ranged from 6.7 (D12.5, group a) to 12.2 (D, group b), with intermediate substrates in a shared group (ab). The pure dark dredging substrate (D, \SI{2.16}{\percent} TOC) supported significantly more leaves than the lower-SOM substrates, indicating that tiller production responds positively to organic enrichment at the high end of the experimental range (0.06--2.16\% TOC) without evidence of a decline. This pattern---broadly increasing with SOM despite scatter among intermediate substrates---has implications for the SOM response curve parameterisation discussed in \cref{sec:results_som_curves}.
...
Kruskal--Wallis tests on individual-level data did not detect a statistically significant overall effect of substrate treatment on any growth metric (height: $\chi^2 = 9.18$, $p = 0.42$; density: $\chi^2 = 5.03$, $p = 0.83$; root biomass: $\chi^2 = 5.17$, $p = 0.82$), owing to the large within-treatment variance among individual plants. However, the treatment-mean response patterns revealed biologically interpretable optima when fitted with Gaussian dose--response curves (\cref{fig:som_response_R_derivation}).

Height growth showed a broad optimum centred at $\mu = 1.32$~\% TOC ($\sigma = 1.52$, amplitude $a = 0.81$; weighted $R^2 = 0.66$), indicating that substrates in the range 0.5--2.0~\% TOC marginally enhance vertical growth relative to pure beach sand, but without a sharply differentiated peak. Stem density growth showed no significant SOM response after removal of one extreme measurement (plant 6b\_5, 97 leaves---nearly five times the next-highest individual, likely caused by multiple seedlings establishing in the same planting box). Without this data point, the Kruskal--Wallis test was non-significant ($\chi^2 = 5.79$, $p = 0.76$) and the NLS Gaussian amplitude was non-significant ($p = 0.14$); density growth was therefore excluded from the Living Dunes parameterisation. Root biomass showed an intermediate pattern with an optimum at $\mu = 1.24$~\% TOC ($\sigma = 0.68$, $a = 0.65$; weighted $R^2 = 0.54$). Full coefficient tables are provided in \cref{sec:appendix_tables}.
...
\input{tables/som_derived_parameters}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{som_response_R_derivation.pdf}
    \caption{SOM response curves for \textit{Ammophila arenaria} derived from SUSANA common-garden treatment means. Grey points show individual plant-level observations; coloured points with error bars show treatment means $\pm$ SE; solid lines show the fitted Gaussian (a--c) and logistic (d) curves. (a)~Height growth shows a broad optimum at $\mu = 1.32$\,\% TOC. (b)~Density growth: one extreme measurement removed (see text); no significant SOM effect. (c)~Root biomass growth peaks at $\mu = 1.24$\,\% TOC. (d)~Mortality shows no significant SOM effect ($\beta_1 = 0.010$, $p = 0.97$).}
    \label{fig:som_response_R_derivation}
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{som_response_curves.pdf}
    \caption{Parameterised SOM response curves as implemented in the Living Dunes vegetation model. (a)~Process-specific Gaussian boost curves for height growth and the root-proxy processes lateral spread and shoot formation, derived from the experimental fits in \cref{fig:som_response_R_derivation}. (b)~Geometric mean of all active SOM factors, representing the combined growth multiplier applied in the model. Shaded band: experimental SOM range (0.10--3.72\,\% SOM; 0.06--2.16\,\% TOC). Vertical dotted line: reference beach sand baseline (substrate R, 0.10\,\% SOM). Curves use the anchored boost formulation $f(\mathrm{SOM})/f(\mathrm{SOM}_{\mathrm{ref}})$, which guarantees $f(\mathrm{SOM}_{\mathrm{ref}})=1$ exactly. Density growth SOM factor excluded (non-significant after removal of one extreme measurement).}
    \label{fig:som_response_modelled}
\end{figure}
...
The height and root growth optima ($\mu = 1.32$ and $1.24$~\% TOC, respectively) were broad ($\sigma > 0.6$), indicating that these traits respond gradually to SOM enrichment within the experimental range. Because no significant SOM--density relationship could be established after removal of one extreme measurement, stem density growth was excluded from the Living Dunes SOM parameterisation, reducing the model's demographic SOM effects to height growth and root biomass (the latter serving as a proxy for lateral spread and shoot formation). Future monitoring may reveal a significant density response with larger sample sizes.
...
This work was supported by the SUSANA project (Sustainable Use of Sand in Nature-based Solutions; VLAIO grant HBC.2022.0548). We thank the Flemish Maritime Services Division (MDK) for access to the study site and dredging material, and the port authority of Zeebrugge for hosting the field experiments. \todo{Complete acknowledgements.}
