...
$^{3}$Department of Civil Engineering, KU Leuven, Technology Campus Bruges, Belgium \\[0.5em]
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../../output/substrate/TOC.PNG}
    ...
    \begin{subfigure}[b]{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../../output/germination/soil moisture boxplots.png}
...
The saturated sand transport rate is computed following \citet{bagnold1941physics}:
\begin{equation}
...
\begin{itemize}
    \item Infiltration is limited by the saturated hydraulic conductivity (\SI{200}{\milli\metre\per\hour} for beach sand);
    \item Drainage from the surface layer follows an exponential decay toward field capacity;
    \item Capillary rise from the deep layer to the surface layer is modelled as a function of the moisture gradient between layers;
    \item Plant water uptake follows a stress--response function based on root-zone moisture relative to field capacity and wilting point.
\end{itemize}
...
The storm boundary conditions (water level, wave height, wave period) were based on a design storm calibrated to the hindcast of Storm Corrie (January 2022), which produced significant wave heights exceeding \SI{4}{\metre} and elevated water levels at the study site. Key XBeach parameters were set as follows:
\begin{itemize}
    \item Sediment: $D_{50}$ and $D_{90}$ set per substrate scenario from the measured grain size distributions;
    \item Vegetation: aboveground biomass properties (height, stem diameter, stem density, drag coefficient $C_d$) were derived from the marram grass cover density predicted by Living Dunes at year 3. The drag coefficient was set to a recommended value of $C_d = 1.0$ following \citet{mendez2004estimation};
    \item Root cohesion: implemented as additional soil strength in the root zone, with root depth derived from the Living Dunes allometric relationship (root depth = $\sqrt{\rho_v} \times d_{max}$ where $d_{max} = \SI{1.5}{\metre}$);
    \item Hydrodynamic parameters: \texttt{facua} (wave-skewness transport), \texttt{gamma} (breaker parameter), \texttt{gammax} (maximum wave height-to-depth ratio), \texttt{eps} (minimum wet depth), \texttt{wetslp}/\texttt{dryslp} (critical avalanche slopes), \texttt{facSk}/\texttt{facAs} (skewness/asymmetry calibration factors), and \texttt{bedfriccoef} were all set to calibrated values from the Storm Corrie hindcast;
    \item Sediment transport: Soulsby--Van Rijn formulation with default parameters;
    \item Seaward bathymetry: extended using measured profiles from the BPNS.
\end{itemize}
...
Simulation outputs were evaluated using the following metrics:
\begin{itemize}
    \item \textbf{Dune volume}: total sand volume above the non-erodible layer, integrated over the restoration footprint;
    \item \textbf{Crest height}: maximum elevation along the cross-shore profile;
    \item \textbf{Vegetation cover}: spatially averaged marram grass cover density at the end of the growing season;
    \item \textbf{Storm erosion volume}: total volume loss during the XBeach storm simulation;
    \item \textbf{Recovery rate}: dune volume gain per year during the post-storm Living Dunes simulation;
    \item \textbf{Aeolian transport activity}: percentage of time steps with non-zero transport, mean transport rate, and dominant transport direction;
    \item \textbf{Moisture retention index}: time-averaged surface soil moisture content as a proxy for SOM-driven ecohydrological enhancement.
\end{itemize}
...
Marram grass growth was strongly influenced by substrate type (ANOVA: $F_{9,981} = 10.76$, $p < 10^{-15}$ for leaf length; $F_{9,1078} = 7.87$, $p < 10^{-10}$ for number of leaves). Growth rates over time were not significantly affected by substrate (non-significant substrate $\times$ time interaction), indicating that the substrate effect manifested as an initial size difference rather than a differential growth trajectory.

Plants growing on the reference beach sand (R) produced significantly shorter leaves than those on all alternative substrates ($p < 0.05$, Tukey's HSD), with a mean height deficit of \SI{11.3}{\centi\metre} relative to the pure dark substrate (D) at the end of the growing season. The D/W mixtures with intermediate dark fraction (D50 and D37.5) produced the tallest plants, exceeding even the pure dark substrate by \SI{5.8}{\centi\metre} and \SI{5.3}{\centi\metre}, respectively ($p < 0.05$).

The number of living leaves followed a distinct pattern: pure D produced significantly more leaves than all other substrates from the beginning of the growing season ($p < 0.05$), while no significant differences were observed among the remaining substrates. This immediate differentiation after planting suggests that the nutrient-rich dark substrate promoted rapid recovery from transplant stress.

\begin{figure}[htbp]
    \centering
    \begin{subfigure}[b]{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../../output/common_garden/aboveground_biomass/marram_height_loess.png}
        \caption{Marram height (LOESS)}
        \label{fig:height_loess}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../../output/common_garden/aboveground_biomass/leaves_number_loess.png}
        \caption{Number of leaves (LOESS)}
        \label{fig:leaves_loess}
    \end{subfigure}
    \caption{Aboveground growth trajectories during the common-garden experiment. The smoothed curves highlight the divergence over time, with intermediate mixtures producing optimal height while pure dark sediment (D) supports the most leaves. For full distributions and detailed boxplots, see \cref{sec:appendix_figures}.}
    \label{fig:aboveground_growth}
\end{figure}

\subsubsection{Germination Experiment}

Germination success was significantly lower on the reference beach sand (R) compared to all alternative substrates (ANOVA: $p < 0.05$), consistent with observed seed loss through wind erosion on the bare, cohesionless sand surface. Seedling growth showed a significant substrate $\times$ time interaction ($p < 0.05$), indicating differential growth trajectories among substrates. Seedlings on R performed substantially worse than those on any alternative substrate throughout the monitoring period.

The microclimate loggers revealed systematic differences in soil moisture among substrates: moisture content was highest in pure D, followed by D75, D50, D25, R, and W, consistent with the expected positive correlation between organic matter content and water retention. Soil temperature in D was slightly elevated compared to other substrates, attributable to its darker colour and higher heat absorption.

\begin{figure}[htbp]
    \centering
    \begin{subfigure}[b]{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../../output/germination/percentage of germinated seeds.png}
        \caption{Germinated seeds percentage}
        \label{fig:germ_pct}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../../output/germination/length of seedlings.png}
        \caption{Seedling length}
        \label{fig:seedling_length}
    \end{subfigure}
    \caption{Germination success and initial establishment across the tested substrates. Germination and seedling extension were significantly inhibited on the reference beach sand (R) compared to the alternative substrates.}
...
The positive feedback operates as follows: SOM increases moisture retention (Pathway 1), which enhances vegetation growth (\cref{eq:veg_growth} via the moisture response function), which in turn produces more litter and root exudates that further increase SOM. Simultaneously, the vegetation canopy reduces surface wind shear through aerodynamic roughness, allowing the disturbance index to decay toward zero (Pathway 3), which maximises the organic cohesion contribution (Pathway 2), further reducing erosion. This positive feedback is stabilising: it drives the system toward a vegetated, cohesive, erosion-resistant state.
...
The balance between these feedbacks depends critically on the SOM content and the local wind climate. Under the moderate wind regime of the Belgian coast (mean gust-corrected speed \SI{7.1}{\metre\per\second}), our preliminary results suggest that the negative feedback dominates above approximately \SI{3}{\percent} TOC, where the threshold shear velocity increase suppresses transport during all but the most energetic events.
...
Simulations of 10 substrate scenarios at Ostend reveal that moderate SOM enrichment (\SIrange{1}{3}{\percent} TOC) optimises long-term dune development by simultaneously enhancing vegetation establishment and maintaining sufficient aeolian transport for continued dune growth. Excessive organic content ($>$\SI{5}{\percent} TOC) suppresses aeolian processes and limits dune volume accumulation despite superior initial vegetation performance.
...
\input{SOM_dune_development_appendix}
